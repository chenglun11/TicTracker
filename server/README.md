...
当前服务端已切换为 SQLite 主存储：`sync.json` 只作为旧客户端兼容输入/导出格式，启动时会自动导入旧 `data/sync.json`，原文件保留并额外写入一份备份。
...
`config.yaml` 含密钥；`data/tictacker.sqlite3`、旧 `data/sync.json` 与 `data/backups/` 含飞书 webhook secret。
...
服务端通过 token 解析 workspace，不在 URL 路径暴露 workspace ID。第一版保留默认 workspace，并支持在 SQLite 中扩展更多 workspace/token。
...
| Workspace Token 解析租户，sync/web token 分权 | `tenant.go: WorkspaceAuthMiddleware` |
...
- `legacy sync.json imported into sqlite` — 旧数据完成 SQLite 导入
...
- Workspace 管理接口：第一版以 token 解析租户和默认 workspace 为主，新增 workspace 可通过 SQLite 初始化/后台脚本接入
