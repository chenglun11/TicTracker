...
final class JiraService {
    ...
    func syncTrackedIssues() async {
        ...
        for issue in store.trackedIssues {
            ...
            // 已排期 / 待验收 / 观测中属于人工推进的状态，禁止 Jira 自动回退到"待处理"（但允许向前推进到处理中/测试中/已修复等）
            let blockRegressToPending = (issue.status == .scheduled || issue.status == .pendingAcceptance || issue.status == .observing) && newStatus == .pending
