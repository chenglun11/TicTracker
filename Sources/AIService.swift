...
final class AIService {
    ...
    func generateWeeklyReport(rawReport: String, config: AIConfig) async throws -> String {
        print("[AI] 开始生成周报，服务商: \(config.provider.rawValue)")
        ...
        do {
            ...
            print("[AI] 周报生成成功，长度: \(result.count) 字符")
            return result
        } catch {
            print("[AI] 周报生成失败: \(error.localizedDescription)")
            throw error
        }
    }
    ...
    private func callClaude(apiKey: String, config: AIConfig, system: String, user: String) async throws -> String {
        ...
        print("[AI] 调用 Claude API: \(url.absoluteString), 模型: \(config.effectiveModel)")
        ...
        guard let http = response as? HTTPURLResponse else {
            print("[AI] 无效的 HTTP 响应")
            throw AIError.invalidResponse
        }
        print("[AI] HTTP \(http.statusCode), \(data.count) bytes")
        guard http.statusCode == 200 else {
            let msg = String(data: data, encoding: .utf8) ?? "HTTP \(http.statusCode)"
            print("[AI] Claude API 错误: \(msg)")
    ...
    private func callOpenAI(apiKey: String, config: AIConfig, system: String, user: String) async throws -> String {
        ...
        print("[AI] 调用 OpenAI API: \(url.absoluteString), 模型: \(config.effectiveModel)")
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
