...
团队模式下，线上服务是唯一权威数据源。每次写入都会推进 workspace 的 `revision`：

- Web、macOS 和其他客户端读取同一个线上修订版。
- macOS 提交快照时必须携带上次读取到的 `If-Match`，服务端拒绝过期提交，避免多人修改互相覆盖。
- Web 工作台每 5 秒探测一次轻量同步元数据；revision 变化时自动刷新问题和统计。
- macOS 的 HTTP API 模式最多每 30 秒检查一次线上变化；本地与线上同时修改时保留本地内容并明确提示冲突。
- revision 同时记录最后修改来源；Web 可查看用户名或 macOS 客户端 ID。上传响应丢失后，客户端会按内容指纹识别线上已包含该提交并自动恢复同步基线。
- iCloud 与 WebDAV 仍保留原有个人文件同步语义，不用于多人协作的权威数据源。
